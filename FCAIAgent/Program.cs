...
static class My
{
    ...
    public static async Task RunAsync(AIAgent agent, ChatMessage chatMessage, AgentSession? session = null)
    {
        try {
            var response = await agent.RunAsync(chatMessage, session ?? await agent.CreateSessionAsync(), null);
